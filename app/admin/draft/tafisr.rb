...
ActiveAdmin.register Draft::Tafsir do
  ...
  action_item :next, only: :show do
    if item = resource.next_ayah_tafsir
      link_to "Next(#{item.start_verse.verse_key})", "/cms/draft_tafsirs/#{item.id}"
    end
  end
    ...
    attributes_table do
      ...
      row :group_tafsir do |resource|
        if resource.group_tafsir
          div do
            span link_to(resource.group_tafsir.verse_key, [:cms, resource.group_tafsir]), class: 'tw-mr-4'
      ...
      tafsirs.each do |t|
        ...
        div class: "tw-w-full tw-p-2 tw-border-b tw-mb-3 #{'selected' if selected == resource_content.id}" do
          ...
          div class: 'tw-flex tw-my-2 tw-justify-between tw-gap-2 tw-flex-wrap' do
            span(link_to 'Filter', "/cms/draft_tafsirs?q%5Bresource_content_id_eq%5D=#{resource_content.id}", class: 'btn btn-info btn-sm')
            ...
            if can?(:manage, :draft_content)
              ...
              span(link_to 'Validate', validate_draft_cms_resource_content_path(resource_content), class: 'btn btn-success btn-sm', data: { controller: 'ajax-modal', url: validate_draft_cms_resource_content_path(resource_content) })
              span(link_to 'Compare grouping', compare_ayah_grouping_cms_resource_content_path(resource_content), class: 'btn btn-success btn-sm', data: { controller: 'ajax-modal', url: compare_ayah_grouping_cms_resource_content_path(resource_content), css_class: 'modal-lg' })

              span(link_to 'Approve', import_draft_cms_resource_content_path(resource_content, approved: true), method: 'put', class: 'btn btn-warning btn-sm', data: { confirm: 'Are you sure to import this tafsir?' })
              span(link_to 'Delete', import_draft_cms_resource_content_path(resource_content, remove_draft: true), method: 'put', class: 'btn btn-danger btn-sm', data: { confirm: 'Are you sure to remove draft tafsir?' })
            end
          end
        end
      end
