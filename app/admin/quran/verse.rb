...
ActiveAdmin.register Verse do
  ...
  show do
    ...
    panel "<div data-bs-toggle='collapse' data-bs-target='#recitations' class='flex collapable'>Available Recitations(#{verse.audio_files.size}) <span class='ms-auto'></span></div>".html_safe do
    ...
    panel "<div data-bs-toggle='collapse' data-bs-target='#translations' class='flex collapable scrollable'>Translations(#{verse.translations.size}) <span class='ms-auto'></span></div>".html_safe do
            ...
            tr do
              ...
              td class: 'quran-text qpc-hafs' do
                matching_verse.words.map do |w|
                  span w.text_qpc_hafs, class: "#{'text-green-500' if positions.include?(w.position)}"
                end
              end
            end
          end
